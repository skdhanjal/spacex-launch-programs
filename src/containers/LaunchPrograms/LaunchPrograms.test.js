...
jest.mock('../../services/LaunchManager');

test.skip('should render launch programs container', async () => {
  const launches = [
  ...
  getLauchesList.mockResolvedValueOnce(launches);
  const { container, getByText } = render(<LaunchPrograms></LaunchPrograms>);

  expect(container.firstChild).toHaveClass('launch-programs');
